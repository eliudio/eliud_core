...
class MaintainBlockingListWidget extends StatefulWidget {
  ...
  MaintainBlockingListWidget({ Key? key, required this.app,  }): super(key: key);
...
class MaintainBlockingListWidgetState extends State<MaintainBlockingListWidget> {
  @override
  Widget build(BuildContext context) {
    return BlocBuilder<AccessBloc, AccessState>(
        builder: (context, accessState) {
      if (accessState is AccessDetermined) {
        return BlocBuilder<MaintainBlockingListBloc, MaintainBlockingListState>(builder: (context, state) {
  ...
  Widget theList(BuildContext context, List<BlockedMember> values, AccessState accessState) {
    return Container(
...
class BlockingListItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if ((value.memberPublicInfoModel.photoURL== null)) {
      ...
      name = ((value.memberPublicInfoModel.name != null) && (value.memberPublicInfoModel.name!.length > 0)) ? value.memberPublicInfoModel.name! : 'No name';
    }
    return Dismissible(
