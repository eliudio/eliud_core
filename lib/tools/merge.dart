...
List<T> merge<T>(List<T> list1, List<T> list2) {
  List<T> output = [];
  var min_length = min(list1.length, list2.length);
  var max_length = max(list1.length, list2.length);
...
List<T> mergeUnique<T>(List<T> list1, List<T> list2, IsEqual isEqual) => removeDuplicates(merge(list1, list2), isEqual);
