...
class DialogFirestore implements DialogRepository {
  ...
  Future<DialogModel?> get(String? id, {Function(Exception)? onError}) {
    return DialogCollection.doc(id).get().then((doc) async {
      if (doc.data() != null)
...
//    In the meantime:
      stream = getQuery(appRepository()!.getSubCollection(appId, 'dialog'), orderBy: orderBy,  descending: descending,  startAfter: startAfter as DocumentSnapshot?,  limit: limit, privilegeLevel: privilegeLevel, eliudQuery: eliudQuery, appId: appId)!.snapshots().map((data) {
      Iterable<DialogModel?> dialogs  = data.docs.map((doc) {
        DialogModel? value = _populateDoc(doc);
        return value;
      }).toList();
      return dialogs as List<DialogModel?>;
    });
...
//  see comment listen(...) above
    stream = getQuery(appRepository()!.getSubCollection(appId, 'dialog'), orderBy: orderBy,  descending: descending,  startAfter: startAfter as DocumentSnapshot?,  limit: limit, privilegeLevel: privilegeLevel, eliudQuery: eliudQuery, appId: appId)!.snapshots()
        .asyncMap((data) async {
  ...
  Stream<List<DialogModel?>> values({String? orderBy, bool? descending, Object? startAfter, int? limit, SetLastDoc? setLastDoc, int? privilegeLevel, EliudQuery? eliudQuery }) {
    ...
    Stream<List<DialogModel?>> _values = getQuery(DialogCollection, orderBy: orderBy,  descending: descending,  startAfter: startAfter as DocumentSnapshot?, limit: limit, privilegeLevel: privilegeLevel, eliudQuery: eliudQuery, appId: appId)!.snapshots().map((snapshot) {
