...
typedef PolicyWidgetProvider(PolicyModel value);

class PolicyListWidget extends StatefulWidget with HasFab {
  BackgroundModel listBackground;
  PolicyWidgetProvider widgetProvider;
  bool readOnly;
  String form;
  PolicyListWidgetState state;
  bool isEmbedded;

  PolicyListWidget({ Key key, this.readOnly, this.form, this.widgetProvider, this.isEmbedded, this.listBackground }): super(key: key);

  @override
  PolicyListWidgetState createState() {
    state ??= PolicyListWidgetState();
    return state;
  }
...
class PolicyListWidgetState extends State<PolicyListWidget> {
  PolicyListBloc bloc;

  @override
  void didChangeDependencies() {
    bloc = BlocProvider.of<PolicyListBloc>(context);
  ...
  Widget build(BuildContext context) {
    var accessState = AccessBloc.getState(context);
    if (accessState is AppLoaded) {
      return BlocBuilder<PolicyListBloc, PolicyListState>(builder: (context, state) {
        if (state is PolicyListLoading) {
          return Center(
            child: DelayedCircularProgressIndicator(),
          );
        } else if (state is PolicyListLoaded) {
        ...
        itemBuilder: (context, index) {
          ...
          return PolicyListItem(
            ...
            onDismissed: (direction) {
              BlocProvider.of<PolicyListBloc>(context)
                  .add(DeletePolicyList(value: value));
              Scaffold.of(context).showSnackBar(DeleteSnackBar(
                message: "Policy " + value.documentID,
                onUndo: () => BlocProvider.of<PolicyListBloc>(context)
                    .add(AddPolicyList(value: value)),
              ));
            },
            onTap: () async {
                                   final removedItem = await Navigator.of(context).push(
                        pageRouteBuilder(accessState.app, page: BlocProvider.value(
                              value: BlocProvider.of<PolicyListBloc>(context),
  ...
  Widget getForm(value, action) {
    if (widget.form == null) {
      return PolicyForm(value: value, formAction: action);
    } else {
      return null;
    }
  }
...
class PolicyListItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Dismissible(
      key: Key('__Policy_item_${value.documentID}'),
      onDismissed: onDismissed,
      child: ListTile(
        onTap: onTap,
        title: Hero(
          tag: '${value.documentID}__PolicyheroTag',
          child: Container(
