...
import 'blocking_list_bloc.dart';
import 'blocking_list_event.dart';
...
class BlockingComponentSelector extends ComponentSelector {
  ...
  Widget createSelectWidget(BuildContext context, AppModel app, int privilegeLevel, double height,
    ...
    return BlocProvider<BlockingListBloc>(
          create: (context) => BlockingListBloc(
          eliudQuery: getComponentSelectorQuery(0, app.documentID),
          blockingRepository:
              blockingRepository(appId: appId)!,
          )..add(LoadBlockingList()),
      child: SelectBlockingWidget(app: app,
          height: height,
...
class SelectBlockingWidget extends StatefulWidget {
  ...
  @override
  _SelectBlockingWidgetState createState() {
    return _SelectBlockingWidgetState();
  }
}

class _SelectBlockingWidgetState extends State<SelectBlockingWidget> with TickerProviderStateMixin {
  ...
  void _handlePrivilegeTabSelection() {
    if ((_privilegeTabController != null) &&
        ...
        BlocProvider.of<BlockingListBloc>(context).add(
            BlockingChangeQuery(newQuery: getComponentSelectorQuery(_currentPrivilege, widget.app.documentID)));
    }
  }

  Widget theList(BuildContext context, List<BlockingModel?> values) {
    var app = widget.app; 
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<BlockingListBloc, BlockingListState>(
      ...
      if ((state is BlockingListLoaded) && (state.values != null)) {
