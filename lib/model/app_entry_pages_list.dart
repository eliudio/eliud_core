...
typedef AppEntryPagesWidgetProvider = Function(AppEntryPagesModel? value);

class AppEntryPagesListWidget extends StatefulWidget with HasFab {
  final AppModel app;
  final BackgroundModel? listBackground;
  final AppEntryPagesWidgetProvider? widgetProvider;
  final bool? readOnly;
  final String? form;
  //final AppEntryPagesListWidgetState? state;
  final bool? isEmbedded;

  AppEntryPagesListWidget(
      {super.key,
      required this.app,
      this.readOnly,
      this.form,
  ...
  AppEntryPagesListWidgetState createState() {
    return AppEntryPagesListWidgetState();
  }

  @override
  Widget? fab(BuildContext context) {
    if ((readOnly != null) && readOnly!) return null;
    var state = AppEntryPagesListWidgetState();
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<AccessBloc, AccessState>(
      ...
      if (accessState is AccessDetermined) {
        return BlocBuilder<AppEntryPagesListBloc, AppEntryPagesListState>(
...
//            app: accessState.app,
                ...
                onTap: () async {
                  final removedItem = await Navigator.of(context).push(
                      pageRouteBuilder(widget.app,
                          page: BlocProvider.value(
                              value: BlocProvider.of<AppEntryPagesListBloc>(
                                  context),
      ...
      child: ListTile(
        ...
        title: Center(
            child: StyleRegistry.registry()
                ...
                .frontEndStyle()
                .textStyle()
                .text(app, context, value.entryPage!.documentID)),
        subtitle: value.entryPage!.title != null
            ? Center(
                child: StyleRegistry.registry()
                    .styleWithApp(app)
                    .frontEndStyle()
                    .textStyle()
                    .text(app, context, value.entryPage!.title!))
            : Container(),
      ),
    );
  }
