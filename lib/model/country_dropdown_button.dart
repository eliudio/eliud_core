...
typedef CountryChanged(String? value, int? privilegeLevel,);

class CountryDropdownButtonWidget extends StatefulWidget {
  final AppModel app;
  int? privilegeLevel;
  String? value;
  final CountryChanged? trigger;
  final bool? optional;

  CountryDropdownButtonWidget({ required this.app, this.privilegeLevel, this.value, this.trigger, this.optional, Key? key }): super(key: key);

  @override
  State<StatefulWidget> createState() {
    return CountryDropdownButtonWidgetState();
  }
}

class CountryDropdownButtonWidgetState extends State<CountryDropdownButtonWidget> {
  CountryListBloc? bloc;

  CountryDropdownButtonWidgetState();
...
var app = widget.app;
var widgets = <Widget>[];
widgets.add(value.countryCode != null ? Center(child: StyleRegistry.registry().styleWithApp(app).frontEndStyle().textStyle().text(app, context, value.countryCode!)) : Container());
widgets.add(value.countryName != null ? Center(child: StyleRegistry.registry().styleWithApp(app).frontEndStyle().textStyle().text(app, context, value.countryName!)) : Container());
return widgets;
}
        ...
        return ListView(
          ...
          Row(children: [(false)
            ? Container(
                ...
                child: DropdownButton<String>(
                      ...
                      items: items,
                      value: widget.value,
                      hint: text(widget.app, context, 'Select a country'),
          ...
          if (widget.value != null) 
            Align(alignment: Alignment.topRight, child: button(
              ...
              label: 'Update',
              onPressed: () {
                updateComponent(context, widget.app, 'countrys', widget.value, (newValue) {
                  setState(() {
  ...
  void _onPrivilegeLevelChange(int? value) {
    BlocProvider.of<CountryListBloc>(context).add(CountryChangeQuery(
