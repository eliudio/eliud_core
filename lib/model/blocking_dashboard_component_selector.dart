...
import 'blocking_dashboard_list_bloc.dart';
import 'blocking_dashboard_list_event.dart';
...
class BlockingDashboardComponentSelector extends ComponentSelector {
  ...
  Widget createSelectWidget(BuildContext context, AppModel app, int privilegeLevel, double height,
    ...
    return BlocProvider<BlockingDashboardListBloc>(
          create: (context) => BlockingDashboardListBloc(
          eliudQuery: getComponentSelectorQuery(0, app.documentID),
          blockingDashboardRepository:
              blockingDashboardRepository(appId: appId)!,
          )..add(LoadBlockingDashboardList()),
      child: SelectBlockingDashboardWidget(app: app,
          height: height,
...
class SelectBlockingDashboardWidget extends StatefulWidget {
  ...
  @override
  _SelectBlockingDashboardWidgetState createState() {
    return _SelectBlockingDashboardWidgetState();
  }
}

class _SelectBlockingDashboardWidgetState extends State<SelectBlockingDashboardWidget> with TickerProviderStateMixin {
  ...
  void _handlePrivilegeTabSelection() {
    if ((_privilegeTabController != null) &&
        ...
        BlocProvider.of<BlockingDashboardListBloc>(context).add(
            BlockingDashboardChangeQuery(newQuery: getComponentSelectorQuery(_currentPrivilege, widget.app.documentID)));
    }
  }

  Widget theList(BuildContext context, List<BlockingDashboardModel?> values) {
    var app = widget.app; 
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<BlockingDashboardListBloc, BlockingDashboardListState>(
      ...
      if ((state is BlockingDashboardListLoaded) && (state.values != null)) {
