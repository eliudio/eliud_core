...
class PosSizeJsFirestore implements PosSizeRepository {
  ...
  Stream<List<PosSizeModel>> values({String orderBy, bool descending }) {
    if (orderBy == null) {
      ...
      return posSizeCollection.orderBy(orderBy, descending ? 'desc': 'asc').onSnapshot
          .map((data) => data.docs.map((doc) => _populateDoc(doc)).toList());
    }
  }
  ...
  @override
  Future<List<PosSizeModel>> valuesList({String orderBy, bool descending }) {
    if (orderBy == null) {
      ...
      return posSizeCollection.orderBy(orderBy, descending ? 'desc': 'asc').get().then((value) {
        var list = value.docs;
        return list.map((doc) => _populateDoc(doc)).toList();
      });
    }
